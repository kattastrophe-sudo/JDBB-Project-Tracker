...
export const ProgressMatrix = ({ onSelectStudent }) => {
  ...
  const handleStatusUpdate = (newStatus) => { if (!selectedCell) return; const enrollment = enrollments.find(e => e.id === selectedCell.enrollmentId); if (enrollment && enrollment.profileId) { updateProjectStatus(selectedCell.projectId, enrollment.profileId, newStatus); setSelectedCell(null); } };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="overflow-x-auto bg-white dark:bg-slate-900 rounded-lg shadow-sm border border-slate-200 dark:border-slate-800"><table className="w-full text-left border-collapse"><thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700"><tr><th className="p-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider sticky left-0 bg-slate-50 dark:bg-slate-900 z-10 w-48">Student</th>{semesterProjects.map(p => <th key={p.id} className="p-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-center min-w-[100px] border-l border-slate-100 dark:border-slate-800">{p.code}</th>)}</tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-800">{semesterEnrollments.map(enrollment => { const profile = profiles.find(p => p.id === enrollment.profileId); const isPending = !enrollment.profileId; return (<tr key={enrollment.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors"><td className="p-4 sticky left-0 bg-white dark:bg-slate-900 group-hover:bg-slate-50 dark:group-hover:bg-slate-800/50 z-10 border-r border-slate-100 dark:border-slate-800 cursor-pointer" onClick={() => !isPending && onSelectStudent(enrollment.profileId)}><div className="flex items-center gap-3"><span className="font-mono text-xs font-bold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 px-1.5 py-0.5 rounded">{enrollment.tagNumber}</span><div className={`truncate w-32 font-medium transition-colors ${isPending ? 'text-slate-400 italic' : 'text-slate-800 dark:text-slate-200 hover:text-emerald-500'}`} title={profile?.fullName || enrollment.email}>{profile?.fullName || enrollment.email} {isPending && '(Pending)'}</div></div></td>{semesterProjects.map(project => { const status = isPending ? 'not_started' : getStatus(enrollment.profileId, project.id); return (<td key={project.id} className="p-2 border-l border-slate-100 dark:border-slate-800 text-center"><button disabled={isPending} onClick={() => setSelectedCell({enrollmentId: enrollment.id, projectId: project.id})} className={`w-full h-10 rounded text-xs font-bold transition-transform active:scale-95 flex items-center justify-center ${isPending ? 'opacity-30 cursor-not-allowed bg-slate-50 dark:bg-slate-800' : getStatusStyle(status)}`}>{getStatusLabel(status)}</button></td>); })}</tr>); })}</tbody></table></div>
